...
//            System.out.println("Deleted player with id " + db_id);
    ...
    public void setDead(boolean dead, boolean withSync, Trap trap) { setDead(dead, controled, false, trap); }
        ...
        if (!hasReachedExitDoor){
            ...
            if(withSync){
                try {
                    ...
                    req.executeUpdate();
                    req.close();
                    if (trap != null && becomesDead) game.addScoreUponTrapKill(trap, this);
                }
                catch (SQLException err) {
